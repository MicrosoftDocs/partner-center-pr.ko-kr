...
|**열 이름**|**필수 여부**|**설명**|**샘플 값**|
|-----|:-----|:---------|:---|
오류|No|조회에 대 한 만들기/업데이트 작업에 관련 된 내용이이 열에 포함 되는 경우 오류가 발생 합니다. 오류가 여러 개 있는 경우 모두 세미콜론으로 구분 하 여 나열 됩니다.|필수 필드 솔루션 1 누락|
Engagement ID|No|Engagement ID는 Microsoft 파트너 센터 조회 시스템에 의해 생성 됩니다. 새 조회를 만들 때는 필요 하지 않습니다. 레코드를 업데이트 하는 경우 기존 engagement ID를 사용할 수 있습니다.|f7eaae47-0b84-4ac4-b4ea-5b2587d42cee
추천 ID|No|Microsoft 파트너 센터 조회 시스템에서 조회 ID를 생성 합니다. 새 조회를 만들 때는 필요 하지 않습니다. 기존 레코드를 업데이트 하는 경우이를 조회 ID로 채웁니다.|ebacdkdc-0b84-4ac4-b4ea-5b2587d42cee
거래 이름|Yes|참조의 식별 이름입니다.|영국 스프링 거래
고객 이름|Yes|고객 회사의 이름입니다. Microsoft 쪽에서 신속 하 게 일치 하는 조직의 법적 이름을 사용 합니다.|Contoso Corporation
고객 주소 줄 1|Yes|고객 회사의 주소란 1입니다. |한 Contoso 방법
고객 주소 줄 2|No|고객 회사의 주소 줄 2입니다.|NE 148 번
고객 구/군/시|Yes|고객 조직이 있는 구/군/시입니다.|Redmond
고객 시/도|No|고객 조직이 있는 상태입니다.|워싱턴
고객 우편 번호|No|고객 조직이 있는 지역의 우편 번호입니다.|98052
고객 국가|Yes|고객 조직이 있는 국가/지역입니다. [여기]( https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes)에 설명 된 대로 세 문자 국가 코드를 사용 합니다.|미국
고객 D-U-N-S ID|No|고객 조직의 DUNS ID를 인출 해 보세요. 이렇게 하면 Microsoft 쪽에서 고객 조직과 빠르게 일치 하 여 더 빠르게 판매자를 할당 하는 데 도움이 됩니다. 이 [웹 사이트](https://www.dnb.com/duns-number/lookup.html)에서 DUNS ID를 무료로 받을 수 있습니다.|81466849
...
파트너 조회 상태|Yes|회사 관점에서의 거래 상태를 나타냅니다. 조회를 만들거나 수정 하려는 경우에 필요 합니다. 새 거래를 만들려고 하는 경우 **new** 를 사용 합니다. 허용 되는 값은 [여기](https://docs.microsoft.com/partner/develop/referral-resources#referralstatus)에 설명 되어 있습니다.|활성
파트너 조회 하위 상태|Yes|정확한 거래 상태를 나타냅니다. 새 거래를 만들려고 하는 경우 **동의** 함을 사용 합니다. 기존 조회를 수정 하는 경우에도 필요 합니다. 허용 되는 값은 [여기](https://docs.microsoft.com/partner/develop/referral-resources#referralsubstatus)에 설명 되어 있습니다.|수락됨
...
판매 단계|No|이 필드는 조회의 세부 판매 단계를 표시 합니다. [여기](https://aka.ms/salesStages) 에서 판매 단계에 대해 자세히 알아보세요.|40
예상 거래 값|Yes|"고객과의 초기 대화를 기반으로 하는 거래의 가치입니다. 이는 거래가 터미널 상태 중 하나에 도달할 때까지 변경할 수 있습니다.| 성공 또는 손실 됨 "|12563
Currency|Yes|거래 값이 입력 되는 통화입니다. [여기](https://en.wikipedia.org/wiki/ISO_4217)에서 통화 코드를 찾을 수 있습니다.|USD
예상 종료 날짜|Yes|MM/DD/YYYY 형식의 고객에 대 한 초기 대화를 기반으로 하는 거래의 예상 종료 날짜입니다. <br/> **날짜는 UTC 표준 시간대 여야 합니다. 파트너 센터 UI에 표시 되는 모든 날짜는 지역화 된 timezones를 기반으로 합니다. UTC 표준 시간대로 날짜를 제공한 조회를 살펴보면 파트너 센터 UI에 +/-1 일 차이가 있을 수 있습니다.**|1/30/2020
CRM ID|No|CRM 시스템의이 특정 조회에 대 한 식별자입니다 (있는 경우). 자유 형식 텍스트 입력 필드입니다.|34234324-sdfsdf-345345-sfd
마케팅 캠페인 ID|No|이 필드는이 특정 조회를 발생 시킨 마케팅 캠페인을 나타냅니다. 일반적으로 ROI 계산에 사용 됩니다.|BingSummer2020
참고|No|조회와 관련 된 업데이트를 나타내는 자세한 정보|샘플 노트입니다.
Microsoft 도움이 필요 하세요?|Yes|Microsoft에서이 공동 판매 요청을 지원 하도록 할지 여부를 지정 합니다.|Yes
...
Microsoft 영업 팀과 공유|Yes|이는 Microsoft 영업 팀과 거래 정보를 공유 하는지 여부를 나타내는 것입니다. 이는 "Microsoft 도움말이 필요 한가요?" 라는 질문에 대해 아니요를 선택 하는 경우에만 적용 됩니다. "|Yes
Microsoft에 대 한 참고 사항|No|Microsoft에서 도움을 받아야 하는 경우 Microsoft에 대 한 특정 참고 사항|Contoso 고객용 POC에 대 한 도움이 필요
고객/파트너 연락처 공유에 동의|Yes|고객 연락처 세부 정보를 공유 하 고 회사 직원에 게 거래에 대해 작업 하는 세부 정보를 공유 하는 데 동의 합니다. **이 열에 대해 아니요를 선택 하는 경우에는 거래를 만들거나 업데이트할 수 없습니다.** |Yes
해결 방법 1|Yes|솔루션 ID (필수), 통화 값을 입력 하는 통화 (옵션)입니다. [여기](https://en.wikipedia.org/wiki/ISO_4217)에서 통화 코드, sku 가격 (선택 사항) 및 sku 수량 (선택 사항)을 찾을 수 있습니다.  |SOL-1234-PQRS, USD, 10, 100
팀 멤버 1|Yes|각 팀 멤버의 이름, 성, 휴대폰 번호 및 전자 메일 ID를 표시 합니다.| Bob, 파트너, 999999, Bob.partner@Contoso.com
