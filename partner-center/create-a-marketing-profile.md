...
> [!IMPORTANT]  
> 비즈니스 운영을 간소화 하는 파트너를 위해 파트너 센터에서이 사이트에 여러 가지 도구와 웹 사이트 전체에 고르게 퍼집니다 하는 데 사용 하는 특징과 기능을 통합 하 했습니다. 해당 작업의 일부로, 파트너 센터 조회 관련 기능을 모두 이동 되 고 Pinpoint 및 C2PC 사이트 사용이 중지 되었습니다. 또는 사용 하 여 Pinpoint C2PC 과거에서를 가져오고 Microsoft에서 조회를 관리, 조회를 가져오려고 계속 하려면 파트너 센터에서 새 비즈니스 프로필을 만드는 해야 합니다. 더 이상 해당 사이트에서 프로필 가져오기를 지원하지 않습니다. 
...
파트너 센터를 통해 고객과 성공을 보여 하거나 다른 Microsoft 인증 또는 프로그램에 참여 하면 인증 서비스와 해당 성공에 영향을 주는 산업, 특정 제품에 대 한 자동으로 추가 합니다. 보증은 검색 결과에서 중요한 요소입니다.
