...
**내 인센티브 개요**는 각 위치에서 각 프로그램의 상태뿐만 아니라 인센티브 프로그램의 전반적인 상태를 보여 줍니다. 이 정보에는 인센티브 프로그램에 등록한 이후의 총 인센티브 소득 및 지불이 포함됩니다.

또한 이 페이지에는 특정 지역에서 프로그램에 등록하기 위해 수행해야 할 추가 작업뿐만 아니라 위치 또는 프로그램에 따른 소득 및 지불도 포함되어 있습니다.

**내 인센티브 개요**에 액세스하려면:

1. 파트너 센터 [대시보드](https://partner.microsoft.com/dashboard)에 로그인합니다.
2. 메뉴에서 **성과급**, **개요** 를 차례로 선택 합니다.
3. 페이지의 맨 위에서 소득 및 지불 요약을 확인하고 아래 표에서 세부 정보를 확인합니다. 또한 해당 테이블을 정렬 하 고 그룹화 하 고 확장할 수 있습니다.
...
4. 프로그램에 등록하는 데 추가 작업이 필요한 경우 이 정보는 **상태** 열에 표시됩니다. 이 경우 수행해야 할 다음 단계를 알아보려면 펼침 단추 기호를 선택합니다.
...
| 비활성/부적합 | 이 시점에는 동기 프로그램이 등록에 열려 있지 않거나 파트너가 등록 또는 다시 등록에 대 한 현재 자격을 충족 하지 않을 수 있습니다. <br> 상태를 사용할 수 없는 경우 파트너는 프로그램의 현재 자격 **요건을 충족**하지 않습니다. 등록 상태 아래에 있는 **자격 요구 사항 확인** 링크를 선택 하면 자격에 대 한 요구 사항과 충족 된 요구 사항이 표시 됩니다. |
...
|지불                       |**유료** 또는 **보류 중** 을 선택 하 고 원하는 대로 정렬 합니다. 특정 결제에 대 한 자세한 내용을 **보려면 보기**를 선택 합니다. 지급 송금 명세서의 복사본을 다운로드하려면 **다운로드**를 선택합니다. 트랜잭션 기록 데이터를 표시 하는 데 최대 24 시간이 걸릴 수 있으므로 관련 소득을 즉시 볼 수 없습니다.  |
...
데이터를 내보내려는 경우 몇 가지 고려 사항은 다음과 같습니다.

- 대량의 데이터를 제공 하는 경우 처리 될 제출 된 요청에 대 한 SLA는 24 시간입니다.

- 연결 된 MPN Id 및 프로그램에 대 한 데이터만 볼 수 있습니다. 추가 데이터에 액세스 하려는 경우에는 성과급 관리자에 게 문의 하세요.

- [데이터 내보내기] 페이지는 자체적으로 새로 고쳐지지 않습니다. 최신 데이터를 보려면 페이지를 수동으로 새로 고쳐야 할 수도 있습니다.

- 필터를 사용하면 **데이터를 사용할 수 없음** 오류가 발생할 수 있습니다. 이 오류는 기본 기간이 3 개월 이내에 선택 된 후 해당 기간을 벗어난 획득에서 지불 ID를 선택한 경우에 발생할 수 있습니다. 기간을 늘리고 다시 시도합니다.
...
|earningAmountInLastPaymentCurrency   |마지막 지불 통화의 획득 금액입니다.   |
...
|earningType   |요금, 리베이트, 공동 작동, 판매 등 인지를 나타냅니다.   |
|예상 지불 월   |최소 임계값 및 지불 자격 규칙이 충족 되는 경우 예상 금액을 포함 하는 예상 상환 월   |
...
## <a name="transaction-summary-report"></a>트랜잭션 요약 보고서

>[!NOTE]
>트랜잭션 요약 보고서에는 프로그램 또는 소득이 모두 표시 되지 않습니다. 집계 된 레버에만 초점을 맞춘 요약입니다. 

트랜잭션 요약 보고서는 집계 획득 레버에 대해 고객 이름 및 구독 Id를 비롯 한 요약 수준 트랜잭션 정보를 제공 합니다. 이 보고서는 여러 트랜잭션과 연결 된 각 획득 ID를 포함 합니다. 해당 획득 ID에 대 한 총 획득 금액 및 소득 세부 정보는 모든 고객/구독에 대해 반복 됩니다. 이는 집계 레버에 대 한 성과급은 구독 당이 아니라 집합적으로 계산 되기 때문입니다.


### <a name="download-the-transaction-summary-report"></a>트랜잭션 요약 보고서 다운로드

1. **성과급 개요** 페이지의 오른쪽 위 모서리에서 **Money** (지급) 아이콘을 선택 합니다.
2. **데이터 내보내기**를 선택합니다.
3. **트랜잭션 요약** 탭을 선택 합니다.
4. 적절 한 필터를 선택 하 여 집계 소득에 적합 한 프로그램 및 레버를 확인 합니다. 데이터가 표시 되지 않으면 프로그램은 집계 된 레버에 적합 하지 않습니다.
5. **다운로드 시작**   을 선택 하 여 보고서를 내보냅니다.  

>[!NOTE]
>트랜잭션 요약 보고서에는 60 일을 초과 하는 구독에 대 한 고객 및 구독 정보가 포함 되지 않으며 소비 수익이 1 센트 미만인 경우에만 포함 됩니다.
...
|예상 지불 월   |최소 임계값 및 지불 자격 규칙이 충족 되는 경우 예상 금액을 포함 하는 예상 상환 월   |
|  exchangeRateDate | EarningAmount USD를 계산하는 데 사용되는 환율 날짜  |
|  transactionMonth |  트랜잭션 월   |
|  transactionCurrency | 원래 고객 거래가 발생 한 통화  |
|  totalTransactionAmountUSD |  총 트랜잭션   |
|  customerId | 고객 식별자  |
|  customerName | 트랜잭션 고객 이름  |
|  subscriptionId | 고객에 연결된 구독 식별자  |
|  subscriptionStartDate | 구독 시작 날짜  |
