...
이러한 보안 요구 사항과 관련된 사용 약관은 Microsoft 파트너 계약에 포함되어 있습니다. 클라우드 솔루션 공급자 프로그램에 참여하려면 이러한 보안 요구 사항을 구현해야 합니다.
...
### <a name="what-will-happen-if-i-do-not-implement-these-partner-security-requirements"></a>이러한 파트너 보안 요구 사항을 구현하지 않으면 어떻게 되나요?

Microsoft 파트너 계약에서는 사용자 계정에 대해 다단계 인증을 적용하고 파트너 센터 API와 상호 작용하기 위해 안전한 애플리케이션 모델을 채택하도록 요구하고 있습니다. 이러한 보안 실행을 준수하지 않는 파트너는 클라우드 솔루션 공급자 프로그램에서 거래하거나 위임 관리 권한을 이용하여 고객 테넌트를 관리하지 못할 수 있습니다.
...
1. **모든 사용자에 대해 MFA 적용**
    ...
    - Azure MFA는 TOTP(시간 제약이 있는 일회성 암호)를 지원하는 인증 애플리케이션의 유일한 확인 방법을 통해 Azure AD 보안 기본값을 통해 파트너 테넌트의 모든 사용자가 무료로 제공되고 있습니다.
...
예, 각 사용자는 MFA를 적용해야 합니다. Azure AD 보안 기본값을 사용하는 경우 해당 기능이 모든 사용자 계정에 MFA를 적용하므로 추가 작업이 필요하지 않습니다. 그러나 보안 기본값을 사용하면 사용자 계정이 MFA를 준수하고 MFA를 적용할 때 영향을 받지 않는 무료이자 손쉬운 방법입니다.
...
모든 사용자 계정에 대해 다단계 인증을 적용하면 비대화형으로 실행하려는 자동화 또는 통합에 영향을 미칩니다. 파트너 보안 요구 사항에 따라 파트너 센터 API에 대한 보안 애플리케이션 모델을 사용해야 하지만 자동화 및 통합으로 두 번째 인증 요소를 해결하는 데 활용할 수 있습니다. 액세스 중인 리소스는 액세스 토큰 기반 인증을 지원해야 합니다.
