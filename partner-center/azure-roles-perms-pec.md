...
---
# <a name="roles-and-permissions-required-to-earn-partner-earned-credit"></a>파트너의 획득 크레딧을 획득 하는 데 필요한 역할 및 사용 권한
...
Azure Maps 데이터 판독기(미리 보기) | Azure 맵 계정에서 맵 관련 데이터를 읽을 수 있는 액세스 권한을 부여합니다. | 아니요 | 
...
Backup 읽기 권한자 | 백업 서비스를 볼 수 있지만 변경할 수는 없습니다. | 아니요 | 
청구 읽기 권한자 | 결제 데이터에 대해 읽기 권한 허용 | 아니요 | 
...
CDN 엔드포인트 독자 | CDN 엔드포인트를 볼 수 있지만 변경할 수는 없습니다. | 아니요 | 
CDN 프로필 참가자 | CDN 프로필과 해당 엔드포인트를 관리할 수 있지만 다른 사용자에게 액세스 권한을 부여할 수는 없습니다. | 예 | 
CDN 프로필 독자 | CDN 프로필과 해당 엔드포인트를 볼 수 있지만 변경할 수는 없습니다. | 아니요 | 
...
Cognitive Services 데이터 읽기 권한자(미리 보기) | Cognitive Services 데이터를 읽을 수 있습니다. | 아니요 | 
Cognitive Services 사용자 | Cognitive Services의 키를 읽고 나열할 수 있습니다. | 아니요 | 
Cosmos DB 계정 독자 역할 | Azure Cosmos DB 계정 데이터를 읽을 수 있음. Azure Cosmos DB 계정 관리는 DocumentDB 계정 참가자를 참조하세요. | 아니요 | 
...
Cost Management 읽기 권한자 | 비용 데이터 및 구성(예: 예산, 내보내기)을 확인할 수 있음 | 아니요 | 
Data Box 기여자 | 다른 사람에게 액세스 권한을 부여하는 것을 제외한 모든 항목을 Data Box 서비스에서 관리할 수 있습니다. | 예 | 
Data Box 읽기 권한자 | 주문하기나 주문 세부 정보 편집 및 다른 사용자에게 액세스 권한 부여 외에 Data Box 서비스를 관리할 수 있습니다. | 아니요 | 
...
EventGrid EventSubscription 읽기 권한자 | EventGrid 이벤트 구독을 읽을 수 있습니다. | 아니요 | 
...
Log Analytics 독자 | Log Analytics 독자는 모든 Azure 리소스에 대한 Azure 진단의 구성 보기를 비롯하여 모니터링 설정 보기 및 모든 모니터링 데이터를 보고 검색할 수 있습니다. | 아니요 | 
...
Managed Applications 읽기 권한자 | 관리 앱 및 요청 JIT 액세스에서 리소스를 읽을 수 있습니다. | 아니요 | 
관리 ID 참가자 | 사용자 할당 ID를 만들고, 읽고, 업데이트하고, 삭제합니다. | 예 | 
관리 ID 운영자 | 사용자 할당 ID를 읽고 할당합니다. | 예 | 
관리 그룹 참가자 | 관리 그룹 참가자 역할 | 예 | 
관리 그룹 읽기 권한자 | 관리 그룹 읽기 권한자 역할 | 아니요 | 
...
Monitoring Reader | 모든 모니터링 데이터를 읽을 수 있음(메트릭, 로그 등) Azure Monitor에서의 역할, 권한 및 보안 시작도 참조하세요. | 아니요 | 
...
보안 Reader | Security Center에서만: 권장 사항 및 경고 보기, 보안 정책 보기, 보안 상태 보기 가능, 변경 불가 | 아니요 | 
...
Site Recovery 구독자 | Site Recovery 상태를 볼 수 있지만 다른 관리 작업은 수행할 수 없습니다. | 아니요 | 
Spatial Anchors 계정 기여자 | 계정의 공간 앵커를 관리할 수 있지만 삭제할 수는 없습니다. | 예 | 
Spatial Anchors 계정 소유자 | 계정의 공간 앵커를 관리할 수 있고 삭제할 수도 있습니다. | 예 | 
Spatial Anchors 계정 읽기 권한자 | 계정의 공간 앵커 속성을 찾아서 읽을 수 있습니다. | 아니요 | 
...
Storage Blob 데이터 읽기 권한자 | Azure Storage 컨테이너 및 BLOB을 읽고 나열합니다. 특정 데이터 연산에 어떤 작업이 필요한지 알아보려면 BLOB 및 큐 데이터 연산을 호출하기 위한 권한을 참조하세요. | 아니요 | 
...
스토리지 파일 데이터 SMB 공유 Reader | SMB를 통해 Azure 파일 공유에 대 한 읽기 액세스를 허용 합니다. | 아니요 | 
...
Storage 큐 데이터 읽기 권한자 | Azure Storage 큐 및 큐 메시지를 읽고 나열할 수 있습니다. 특정 데이터 연산에 어떤 작업이 필요한지 알아보려면 BLOB 및 큐 데이터 연산을 호출하기 위한 권한을 참조하세요. | 아니요 | 
...
- [파트너 획득 크레딧 - CSP의 새로운 상거래 경험에서 어떻게 작동하는지에 대한 개요](partner-earned-credit.md)
