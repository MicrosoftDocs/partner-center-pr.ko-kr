...
1.  **파트너 센터** 메뉴에서 **서비스 요청** , **고객 요청**을 차례로 선택 합니다. 
...
> [!IMPORTANT]
> Azure에서 고객에 대 한 서비스 요청을 만들어야 하는 경우 다음 사항에 주의 해야 합니다.
>
>- Azure에서 고객에 대 한 서비스 요청을 만들기 위해 간접 공급자는 고객의 Azure 계정에 대 한 액세스 권한을 부여 해야 합니다. 이는 Office 365에 대해 고객을 대신하여 관리하는 것과는 다릅니다. 
>
>- 파트너 센터의 기술 지원팀 관리자는 Azure 서비스 포털에서 서비스 요청을 만들 수 없지만, Azure 서비스 포털에서 지원 그룹을 만들고 해당 그룹에 지원 요청을 기록 하는 권한을 부여 합니다.
...
해당 고객에 대 한 관리자 권한이 없는 경우 고객에 대 한 지원 요청을 만들어야 할 수 있습니다. 이 문제는 다음 두 가지 시나리오 중 하나에서 발생할 수 있습니다. 
 
-   먼저 관계를 설정할 때 관리자 권한을 요청 하지 않았습니다.
-   고객의 Azure 구독을 관리 하 고 다른 사용자는 관리 권한이 없어도 됩니다.
 
이러한 경우 중 하나에서 다음 절차를 사용 하 여 지원 요청을 만들 수 있습니다. 

1. 파트너 센터의 계정 페이지에서 고객의 도메인 이름을 복사 합니다.

2. [Customerdomainname]으로 https://portal.azure.com/ 이동 합니다. 

3. 지원이 필요한 Azure 구독을 선택 합니다.

4. **새 지원 요청**을 선택 하 고 표시 되는 메시지에 따라 요청을 만듭니다. 
...
Microsoft는 Azure Marketplace 제품에 대 한 제품 지원을 제공 하지 않습니다. 지원을 받으려면 제품을 게시 한 ISV (독립 소프트웨어 공급 업체)에 문의 해야 합니다.

ISV의 연락처 정보를 찾으려면 다음을 수행 합니다.

1.  Marketplace 페이지에서 도움이 필요한 제품을 선택 합니다.
2.  제품 페이지에서 지원 연락처 정보를 찾을 수 있습니다. 다음 중 하나 이상이 될 수 있습니다.
    - ISV 웹 사이트의 지원 진입점에 대 한 링크
    - 지원 전자 메일 
    - 지원 연락처 전화 번호

## <a name="faq"></a>FAQ

**지원 권리의 일부로 포함 되는 항목은 무엇 인가요?**

서비스 요청은 파트너 센터를 통해 제출해야 하며 Azure, Office 365, Microsoft Dynamics CRM Online 및 Enterprise Mobility Suite에 사용할 수 있습니다. 클라우드 솔루션 공급자 프로그램에 참여 하는 파트너는 중요 한 문제에 대 한 우선 순위 응답 시간을 예측할 수 있습니다.
...
이 혜택은 유료 또는 평가 기간에 따라 클라우드 솔루션 공급자 프로그램에 참여 하는 모든 파트너에 게 적용 됩니다. 청구 및 구독 관리 지원도 이 패키지의 무료 구성 요소로 포함됩니다.

**초기 응답을 얼마나 빨리 받을 수 있나요?**

최초 응답 시간은 제출한 문제의 심각도에 따라 달라집니다. 문제의 심각도는 서비스 요청을 제출할 때 지정한 비즈니스 영향에 따라 결정됩니다.

**기술적인 고장 수리 인시던트**의 최초 응답 시간은 다음과 같습니다.

-   심각한 영향 (심각도 A): 2 시간 (서비스의 심각한 손실 또는 저하) 프로덕션 서비스가 작동 중지됨)
-   보통 영향 (심각도 B): 4 시간 (보통 서비스 손실 또는 서비스 저하 프로덕션 서비스가 부분적으로 영향을 받음)
-   최소 영향 (심각도 C): 8 시간 (최소한의 서비스 손실 또는 성능 저하) 서비스를 계속 사용할 수 있거나 비프로덕션 서비스가 영향을 받음)
...
**전화로 서비스 요청을 제출할 수 있나요?**

아니요, 이 프로그램에 대한 전화 지원은 제공되지 않습니다.

**Azure Portal에 로그인 하 고 파트너 센터를 바이패스 하면 어떻게 되나요?**
...
CSP 프로그램 지원 자격이 사용자의 파트너 구독에 대 한 지원을 제공 하지 않으므로 자체 파트너 구독과 관련 된 서비스 요청을 만들 때 유효한 지원 계획 자격을 제공 해야 합니다. 예를 들어 MPN 계약 ID, 프리미어, Azure 지원 계획 등이 있습니다. [Azure 지원 FAQ](https://go.microsoft.com/fwlink/?LinkId=717532)를 참조하세요.
...
**Dynamics 365 지원을 추가로 얻을 어떻게 할까요? 있나요?**

다음과 관련 된 문제가 발생 하는 경우 Dynamics 365 구독, 라이선스, 청구, 재무 & 작업, Dynamics 365 제품 라이선스를 계획 하거나 추가 기술 지원이 필요 합니다.
