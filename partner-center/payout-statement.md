...
**지급 명세서에는** 상업용 Marketplace를 통해 판매된 제안의 지급 개요가 표시됩니다. 수익의 트랜잭션 기록을 표시하고, 다음 지급액을 예측하고, 결제 추세를 보여 줍니다. 트랜잭션 기록 및 결제 명세서를 다운로드할 수도 있습니다. 이 문서에서는 지급 명세서에 액세스하는 방법과 파트너 센터 액세스할 수 있는 다양한 지급 페이지 및 다운로드에 대해 설명합니다.
...
지급 명세서에 액세스하려면 계정 **소유자** 또는 **재무 기여자** 역할을 할당받아야 합니다.
...
[파트너 지급 API를](/rest/api/partner-center/partner-payouts) 사용하여 지급 트랜잭션 및 결제 데이터를 직접 연결하고 가져올 수도 있습니다. [지급 서비스 API를 사용하여 지급 관리에서](/partner-center/develop/manage-payouts)자세히 알아보세요.


## <a name="transaction-history"></a>거래 기록

**거래 기록** 페이지에는 지난 36개월 동안의 소득, 예상 다음 지불액 및 소득 및 지불 추세에 대한 요약이 표시됩니다. 이 섹션에서 트랜잭션 세부 정보를 다운로드할 수도 있습니다.<br><br>이 보고서는 아직 보내지 않은 지급액을 포함하여 지급할 수 있는 모든 소득이 표시됩니다. ISV가 파트너 센터 모든 은행 및 세금 정보를 완료하고, >50달러, ISV 계정이 활성 상태이며, 고객에게 요금이 청구되거나(EA 트랜잭션의 경우) 지불이 수신된 경우 수익이 지급됩니다(비 EA 트랜잭션의 경우).
...
트랜잭션이 지급 자격을 충족 하면 트랜잭션이 표시 됩니다. 수익 누락 또는 예상치 못한 수익을 이해 하려면 [상업적 marketplace 지급에 대 한 일반적인 질문](payout-faq.yml#why-are-my-earnings-missing-)을 참조 하세요.
...
다음 표에는 조정을 위한 이유 코드와 해당 설명이 나와 있습니다.
...
**결제** 페이지에는 Microsoft로 획득한 금액이 자세히 설명됩니다. 또한 지불 시기와 정도를 보여줍니다.

>[!Note]
> 지급을 사용할 수 있으려면 수익이 $50의 [결제 임계값](payment-thresholds-methods-timeframes.md)에 도달해야 합니다. 자세한 내용은 Microsoft Publisher [을 참조하세요.](/legal/marketplace/msft-publisher-agreement)
...
- **유료** – 모든 결제가 성공적으로 전송되었습니다. 드롭다운 메뉴에서 연도를 선택하여 해당 연도에 릴리스된 지불액으로 필터링합니다.
- **보류 중** – 예정된 결제.
- **서비스 요금 세금(PDF 양식)** – 서비스 요금 세금이 적용되는 지불에 사용할 수 있습니다. 서비스 요금 세금이 **다른 세금이** 표시 됩니다.
- **보기** – 지불에 포함 된 소득 목록이 포함 된 트랜잭션 기록으로 리디렉션합니다.

수익 누락 또는 예상치 못한 수익을 이해 하려면 [상업적 marketplace 지급에 대 한 일반적인 질문](payout-faq.yml#why-are-my-earnings-missing-)을 참조 하세요.
