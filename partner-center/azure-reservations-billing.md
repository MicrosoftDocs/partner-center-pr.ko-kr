...
일회성 요금 청구서 및 조정 파일에 액세스 하려면 파트너 센터에서 **청구** 를 선택한 다음 **한 번**을 선택 합니다. 
...
|InvoiceNumber |지정한 트랜잭션이 표시되는 송장 번호 |
...
|OrderId |Microsoft 청구 플랫폼에서 주문의 고유 식별자. 지원 팀에 문의할 때는 Azure 예약을 식별하는 것이 유용할 수 있지만, 조정에서는 아닙니다. |
...
|ChargeType |요금 또는 조정 유형. |
|UnitPrice |주문한 각 제품의 가격입니다. |
|수량 |주문한 제품의 수입니다. |
|Subtotal |세금을 적용하기 전의 총액. 할인이 있을 경우 소계가 예상 총액과 일치하는지 확인합니다. |
|TaxTotal |적용 가능한 모든 세금의 총액입니다. |
|Total |총 구입 금액입니다. |
|Currency |통화 형식. 각 청구 엔터티의 통화는 한 가지만 가능합니다. 통화가 첫 번째 송장과 일치하는지 확인한 다음 주요 청구 플랫폼 업데이트 후에 다시 확인합니다. |
