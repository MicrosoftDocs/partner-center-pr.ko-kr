...
감사 또는 고객 참조가 필요 하지 않은 고급 특수화는 다음과 같습니다.
...
3. 고객 참조를 제공 해야 하는 고급 특수화의 이름을 선택 합니다. 예를 들어 **Microsoft Teams에 대 한 모임 및 회의실** 을 선택 하는 경우를 가정해 보겠습니다.
...
## <a name="how-azure-consumption-revenue-acr-is-calculated-for-each-advanced-specialization"></a>각 고급 특수화에 대해 Azure 소비 수익 (ACR)을 계산 하는 방법:

- 파트너-고객 연결에는 고객에 대 한 [레코드 연결의 디지털 파트너](https://support.microsoft.com/topic/link-a-partner-id-for-azure-performance-pal-or-dpor-a8eed43b-82a8-f017-3b1a-f9c8aa385d32) 또는 고객에 대 한 [PAL (파트너 관리자 링크](/azure/cost-management-billing/manage/link-partner-id) ) 연결에 대 한 csp 직접 청구 파트너 또는 csp 간접 재판매인이 포함 됩니다.
- 제품 유형이 평가판 인 구독의 ACR, 혜택 프로그램, 지원 및 내부용은 고급 특수화로 계산 되지 않습니다.

**Azure 소비 수익 필드**

| 필드 이름 | 필드 설명 |
|-------------|----------------------|
| 서비스 수준 1 | Azure 서비스가 번들로 제공 되는 최고 수준 (를 참조 https://azure.microsoft.com/services/) 하세요. |
| 서비스 수준 2 | 두 번째 최고 수준의 Azure 서비스 기반 (예: https://azure.microsoft.com/pricing/) |
| 서비스 수준 3 | 세 번째 최고 수준의 Azure 서비스 기반 (예: https://azure.microsoft.com/pricing/details/virtual-machines/linux/) |
| 서비스 수준 4 | 네 번째 최고 수준의 Azure 서비스 기반 (예: https://azure.microsoft.com/pricing/details/virtual-machines/linux/) |
| 서비스 그룹 2 | 서비스 범주에 따라 Azure 서비스 그룹화 |
| 서비스 그룹 3 | 서비스 하위 범주에 따라 Azure 서비스 그룹화 |
| 서비스 주는 요인은 | 특정 소비 활동에 매핑된 Azure 서비스 그룹 |
| 계산 OS | 운영 체제 계산 |
| 계산 핵심 소프트웨어 | 계산 핵심 소프트웨어 |

**고급 특수화를 위한 Azure 소비 수익 필터**

- Microsoft Azure로 데이터 웨어하우스 마이그레이션
    - Azure Synapse Analytics ACR: 서비스 그룹 3 = SQL DW
- Microsoft Azure의 Kubernetes
    - AKS (Azure Kubernetes Service) ACR: Service 주는 요인은 = AKS, AKS-Engine
    - Azure RedHat OpenShift ACR: 서비스 그룹 3 = ARO, 서비스 수준 2 = 모두 "알 수 없음"을 제외한 모든
- Microsoft Windows 가상 데스크톱
    - Windows 가상 데스크톱 ACR: Service 주는 요인은 = 기본 WVD
- 웹 애플리케이션을 Microsoft Azure로 현대화
    - Azure App Service ACR: 서비스 수준 1 = 계산 및 서비스 수준 2 = Azure App Service
    - Azure 스프링 클라우드 ACR:
        - ACR 조정 유형 = 해당 없음
        - 서비스 그룹 2 = 앱 개발
        - 서비스 그룹 3 = 스프링 클라우드 
        - 서비스 수준 2 = 모두 "알 수 없음"을 제외한 모든

- Microsoft Azure에 대 한 분석
    - Azure Synapse Analytics ACR: 서비스 그룹 3 = SQL DW
    - Data Lake ACR: 서비스 그룹 3 = Azure Data Lake
    - Databricks ACR: Service 그룹 3 = Databricks, Databricks VM
    - Azure Data Factory ACR: Service 그룹 3 = Data Factory  
- Microsoft Azure Stack HCI를 사용 하는 하이브리드 클라우드 인프라
    - Azure Stack HCI ACR: 서비스 수준 2 = Azure Stack HCI
- Microsoft Azure VMware 솔루션
    - Azure VMware 솔루션 (AVS) ACR: 서비스 수준 4 = Azure VMware 솔루션, CloudSimple 별 azure vmware 솔루션 Virtustream, Specialized Compute Azure VMware 솔루션
- Microsoft Azure 호로 하이브리드 운영 및 관리 
    - Arc 사용 서버 ACR: 서비스 수준 4 = Azure Policy
    - Arc 사용 Kubernetes ACR: 서비스 수준 4 = Azure Arc
- Microsoft Azure의 AI 및 Machine Learning
    - AI ACR: 서비스 그룹 2 = AI
...
- Microsoft Azure로 Windows Server 및 SQL Server 마이그레이션
    - Windows ACR 
        - ACR 조정 유형 = 해당 없음, 계산 코어 SW = 코어,
        - Compute OS = WINDOWS, 계산 OS 특성 = WINDOWS, 알 수 없음 또는 계산 OS = LINUX THEN Compute OS Attribute = WINDOWS-AHUB
        - 서비스 수준 2 = Cloud Services, Container Instances, Container Registry, Specialized Compute, Virtual Machines, Virtual Machines Licenses
    - DB (SQL Database) ACR: 서비스 그룹 3 = sqldb
    - SQL Managed Instance (MI) ACR: 서비스 그룹 3 = SQL DB MI
    - SQL VM ACR: 서비스 그룹 3 = iaas의 SQL, iaas VM의 SQL
- Microsoft Azure에 대 한 Linux 및 Open-Source 데이터베이스 마이그레이션
    - VM (Linux Virtual Machines) ACR:
        - 옵션 1:
            - ACR 조정 유형 = N/A, 서비스 수준 2 = Virtual Machines, 계산 os = Linux, 계산 os 특성 = Windows 되지 않음, 알 수 없음, Windows
            - 서비스 수준 4 = Cloud Services ms 시리즈, Virtual Machines ms 시리즈, Virtual Machines ms 시리즈 Windows, Virtual Machines MSv2 시리즈, Virtual Machines MSv2 시리즈 Windows, ms 시리즈 전용 호스트, MSv2 시리즈 전용 호스트를 제외한 모든 것
        - 옵션 2: 서비스 수준 4 에서만 필터를 요구 합니다. 계산 OS/계산 OS 특성과 같은 다른 필드는 필요 하지 않습니다.
            - 서비스 수준 4 = Red Hat Enterprise Linux, Red Hat Enterprise Linux HA, SUSE Linux Enterprise Server 기본, SUSE Linux Enterprise Server for HPC 우선 순위, SUSE Linux Enterprise Server for HPC 표준, SUSE Linux Enterprise Server 우선 순위, SUSE Linux Enterprise Server 표준
    - Azure Database (DB) for MariaDB ACR: 서비스 수준 1 = 데이터베이스 및 서비스 수준 2 = Azure Database for MariaDB
    - MySql ACR 용 Azure DB: 서비스 수준 1 = 데이터베이스 및 서비스 수준 2 = Azure Database for MySQL 및 MySQL Database on Azure
    - postgre SQL ACR 용 Azure DB: 서비스 수준 1 = 데이터베이스 및 서비스 수준 2 = PostgreSQL
    - Azure COSMOS DB ACR: 서비스 수준 1 = 데이터베이스 및 서비스 수준 2 = CosmosDB
- Threat Protection
    - Azure 센티널 ACR: 서비스 수준 4 = 센티널
- 클라우드 보안
    - 하이브리드 환경 XDR 및 네트워크 보안 ACR: 서비스 수준 4 = SQL 용 azure defender, 컨테이너 레지스트리 용 azure defender azure defender for Kubernetes, Application Gateway waf v2, waf Application Gateway, Azure Active Directory B2C, Azure Active Directory Domain Services, 외부 id에 대 한 Storage Azure Active Directory, azure 방호, azure DDOS Protection, azure 방화벽, azure 방화벽 관리자, azure Front 도어 서비스, IoT 용 azure defender, azure 전용 HSM, Key Vault, Network Watcher, azure defender for App Service, azure defender for server, 센티널
