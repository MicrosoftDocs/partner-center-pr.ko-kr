...
**Azure & 클라우드 제품** 페이지에는 사용자에 게 프로 비전 된 두 가지 유형의 구독 혜택이 나열 됩니다.
- 사용 빈도 기반 구독 (Microsoft Azure 크레딧) 
- 라이선스 기반 구독 (예: Office 365)

### <a name="usage-based-subscriptions-with-two-types-of-microsoft-azure-credit"></a>두 가지 유형의 Microsoft Azure 크레딧을 사용한 사용 현황 기반 구독

Microsoft 파트너 네트워크 멤버 자격에 사용할 수 있는 활성 제안에 따라 두 가지 유형의 사용 현황 기반 구독을 받을 수 있습니다. 이러한 활성 제품은 Microsoft Action Pack, 실버 역량 또는 골드 역량을 포함할 수 있습니다. 

이러한 제품에 대해 프로 비전 할 수 있는 두 가지 유형의 사용 현황 기반 구독은 *azure 월별 크레딧* 및/또는 *azure 대량 (매년) 크레딧을* 포함 합니다.

- **Azure 월간 크레딧 프로 비전:** $100/월 (또는 현지 통화와 동일)으로 나타날 수 있습니다.<br/>
*및/또는*
- **Azure 대량 (매년) 크레딧 프로 비전:** $6000/year, $12000/year, and/or $13000/year와 같이 표시 되거나 각각 현지 통화와 동일한 것으로 표시 될 수 있습니다.

파트너 센터에서 Microsoft Azure 구독을 활성화 하려면 회사 (또는 게스트 사용자) 내의 사용자에 게 액세스 권한을 할당 해야 합니다. 사용자에 게 Azure Active Directory 테 넌 트에서 만든 사용자 항목이 이미 있어야 합니다. 그런 후 다음 단계를 수행합니다.

1. 사용자에 게 Azure AD 테 넌 트의 사용자 항목이 있는 경우 파트너 센터 [대시보드에](https://partner.microsoft.com/dashboard)로그인 합니다.

1. 왼쪽 탐색 메뉴에서 **MPN** 을 선택 하 고 그 다음에 **혜택**, **Azure 및 클라우드** 를 차례로 선택 합니다.

1. 활성화 하려는 사용 빈도 기반 구독에 대 한 오른쪽의 **자세히 보기** 열에서 아래쪽 화살표를 선택 합니다. 

1. 파트너 센터 **Azure 및 클라우드 제품** 페이지 (아래)에서 **사용자 할당** 영역을 찾습니다. 그런 다음 **사용자 할당** 옵션을 선택 하 여 해당 사용자에 게 원하는 사용량 기반 구독 혜택을 할당 합니다. 
...
1. 키를 사용할 준비가 되 면이 페이지의 맨 아래에 있는 **활성화 연습** 옵션을 선택 합니다.
   ...
   - 종료 날짜를 연장 하거나 기존 구독에 사용자/라이선스를 추가 하려는 경우 아래 링크를 **개인 또는 incognito 브라우저 창** 에 붙여 넣고 다음 지침을 따르세요.
 
      [https://admin.microsoft.com/AdminPortal/Home#/subscriptions](https://admin.microsoft.com/AdminPortal/Home#/subscriptions)

> [!Note]
> 키가 회수 되 면 같은 키를 두 번 회수할 수 없습니다. 또한 다음과 같은 중요 한 사항에 유의 하세요.
>
> - 라이선스 (사용자)를 추가 해야 하는지 여부, 수 및 종료 날짜를 확장할지 여부를 확인 합니다. 더 많은 라이선스 (사용자)를 추가 하도록 선택 하 고 종료 날짜를 연장 **하지** 않을 경우 시스템은 12 개월 미만의 활성 구독을 제공 합니다. 
>
> - 올바른 자격 증명을 사용 하 여 로그인 하 고 AAD (올바른 Azure Active Directory) 테 넌 트에서 키를 활성화 하 고 있는지 확인 합니다. 올바른 테 넌 트에서 키를 활성화 하려면 항상 개인/incognito 브라우저 창으로 로그인 하세요.
>
> - Microsoft 파트너 센터에 표시 되는 혜택 만료 날짜는 해당 혜택에 대 한 사용 권한을 마지막으로 실행 한 날짜를 나타냅니다. 이 날짜는 M365 관리 센터 포털에 나열 된 구독 종료 날짜와 다를 수 있습니다. 이는 혜택 활성화 날짜, 키와 연결 된 라이선스 수 및 관리 포털에서 추가 되거나 확장 된 라이선스 등의 여러 가지 요인으로 인해 발생할 수 있습니다.

## <a name="extend-the-end-date-or-add-more-licenses-to-an-existing-subscription"></a>종료 날짜를 연장 하거나 기존 구독에 라이선스를 추가 합니다.

기존 구독의 라이선스를 추가 하거나 종료 날짜를 연장 하려면 다음 단계를 따르세요.

1. 아래 링크를 복사 하 여 **개인 또는 incognito 브라우저 창** 에 붙여 넣습니다.

   [https://admin.microsoft.com/AdminPortal/Home#/subscriptions](https://admin.microsoft.com/AdminPortal/Home#/subscriptions)

1. **M365 관리 센터 포털** 에 로그인 하는 경우 올바른 Azure Active Directory 테 넌 트를 사용 하 여 로그인 하는지 확인 합니다. 그런 다음 목록에서 특정 클라우드 구독을 선택 하 고 다음 중 하나를 수행 합니다. 구독을 갱신 하거나 라이선스를 추가 하려면 다음 중 하나를 수행 합니다.

   - 기존 구독을 갱신 하려면 라이선스 영역에서 **종료 날짜 확장** 옵션을 선택 합니다. 종료 날짜를 확장 하면 라이선스 수/사용자 수를 동일 하 게 유지 하면서 구독 기간이 늘어납니다. 
...
2. 표시 된 제품 세부 정보, 라이선스 수 및 구독 기간을 확인 합니다. 준비가 되 면 전환 **을 선택 하 여 구독을 활성화** 합니다.

   M365 관리 센터의 **제품** 탭에서 새로 활성화 된 구독을 사용할 수 있습니다.

## <a name="licensing-scenarios"></a>라이선스 시나리오

이 섹션에서는 Azure 및 클라우드 제품과 연결 된 구독에 대 한 몇 가지 샘플 라이선스 시나리오를 설명 합니다. 이러한 시나리오에서는 구독 종료 날짜를 계산 하는 방법을 알아봅니다.

### <a name="scenario-1"></a>시나리오 1

구독에서 100 라이선스가 활성 상태이 고 25 개의 라이선스가 있는 키를 사용 하 여 종료 날짜를 연장 하려고 하면 100 라이선스를 사용 하는 구독에 대해 종료 날짜가 3 개월로 연장 됩니다.

### <a name="scenario-2"></a>시나리오 2

구독에서 100 라이선스가 활성 상태이 고 100 라이선스의 키를 사용 하 여 종료 날짜를 연장 하려는 경우 종료 날짜는 12 개월 연장 됩니다.

### <a name="scenario-3"></a>시나리오 3

구독에서 5 개 (5) 개의 라이선스가 활성 상태이 고 10 개의 라이선스가 있는 키를 사용 하 여 종료 날짜를 연장 하려고 하면 5 개의 라이선스 (또는 사용자)가 있는 구독에서 종료 날짜가 24 개월 연장 됩니다.

### <a name="scenario-4"></a>시나리오 4

3 ~ 3 개 이상의 활성 라이선스를 포함 하는 키를 사용 하 여 종료 날짜를 연장 하려고 하면 다음 오류 메시지가 표시 됩니다. 구독 종료 날짜는 최대 3 년까지 연장할 수 있으므로이 메시지는 의도적으로 설계 되었습니다.
...
- [Office 365 E3에서 Microsoft 365 Business Premium으로 마이그레이션](/microsoft-365/business/migrate-from-e3) 에 대해 알아봅니다.
   
- [Microsoft 365 Business 음성](/microsoftteams/business-voice/whats-business-voice) 에 대해 자세히 읽기
  
- [비즈니스 음성의 국가 및 지역 가용성](/microsoftteams/business-voice/country-region-availability) 에 대 한 자세한 정보
