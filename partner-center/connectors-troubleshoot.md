...
**적용 내용:** Dynamics 365 CRM | Salesforce CRM

**적절한 역할:** 조회 관리자 | CRM의 시스템 관리자 또는 시스템 사용자 지정자

 ## <a name="questions-and-answers-about-pre-requisites"></a>필수 구성요소에 대한 질문 및 답변

1. 환경에 대한 평가판 공동 판매 추천 커넥터 솔루션을 사용할 수 있나요?

테스트/스테이징 환경에 있는 경우 평가판 솔루션을 선택할 수 있습니다. 유료 버전의 커넥터는 AppSource에서 US$ 15/월로 제공됩니다. 유료 연결을 사용하면 하루에 10K API 호출을 받게 됩니다. 커넥터는 파트너 센터 조회 API를 기반으로 하는 래퍼입니다. 커넥터 솔루션이 파트너 센터 또는 CRM 쪽의 기회에서 **Create** 또는 **Update** 이벤트에 대해 실행되면 API 호출이 이루어집니다.

2. CRM 환경에서 섹션을 만드는 데 필요한 역할은 무엇인가요?

시스템 관리자 또는 시스템 사용자 지정자는 모든 사용자에게 변경 내용을 적용할 수 있습니다. 그러나 모든 앱 사용자는 시스템을 개인 설정할 수 있으며 일부 사용자 지정을 다른 사용자와 공유할 수도 있습니다. 

3. 파트너 판매자가 파트너 센터 작업하기 위해 특별한 역할이 필요한가요?
 
파트너 판매자에게 "조회 관리자" 역할이 할당되어야 합니다. 자세한 내용은 [사용 권한 개요를 참조하세요.](create-user-accounts-and-set-permissions.md)

4. CRM 환경에서 먼저 설정해야 하는 필드는 무엇인가요? 

• 통화가 사용자의 위치에 적합하고 CRM 환경에 정확하게 있는지 확인합니다. • 영업 팀이 CRM 환경에 CRM 사용자로 나열되어야 합니다.
...
E. 흐름 파트너 센터 Webhook 등록에서 **실행을** 선택합니다. 파트너 센터 첫 번째 작업에서 **Salesforce 흐름으로** **http url을** 제공합니다. 이벤트 아래에서 네 가지 옵션을 Select all **등록하고** 덮어쓰기에서 **예를** 선택합니다.

## <a name="questions-and-answers-about-runmaintenance"></a>실행/유지 관리와 대한 질문 및 답변

1. Power Automate 흐름 실행 중 오류를 해결하려면 어떻게 해야 할까요?

Power Automate 흐름이 예상대로 실행되도록 하고 실행 중에 오류를 해결하려면 흐름 [오류 해결을 참조하세요.](/power-automate/fix-flow-failures)

2. 파트너 센터 또는 CRM 환경에서 제대로 동기화되지 않은 조회가 표시되면 어떻게 해야 할까요?
 
조회 동기화의 상태를 확인하려면 **감사를** 선택합니다. 
...
8. 흐름 오류가 없는 경우에도 CRM에 동기화되지 않는 파트너 센터 생성된 공동 판매 기회가 표시되면 어떻게 해야 할까요?
